    ...
    if (response.ok) {
      return StringStream.from(response.body).CSVParse({
          ...
          step: function (results) {
            ...
            if (typeof report[country] === 'undefined') {
              report[country] = {}
            }

            for (let date in row) {
              const newDate = formatDate(date)
              if (typeof report[country][newDate] === 'undefined') {
                report[country][newDate] = {}
              }
              if (typeof report[country][newDate][type] === 'undefined') {
                report[country][newDate][type] = 0
              }

              report[country][newDate][type] += parseInt(row[date], 10)
            }
          }
        })
        .batch(5000)
